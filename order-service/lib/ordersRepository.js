...
async function ensureOrdersSchema() {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_hx_cmd_user ON hx_commandes(id_user)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_hx_cmd_ref ON hx_commandes(reference_publique)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS idx_hx_ret_user ON hx_retours(id_user)`);
}
